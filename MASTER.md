...
A keybind that opens/closes the whole menu (§14) is just an ordinary `CreateKeybind` with a `Flag` — no special-cased "menu keybind" concept exists. Because it's Flag'd like any other control, the chosen bind itself round-trips through `SaveConfig`/`LoadConfig` for free.
...
| **Config Manager** (`CreateConfigManager`) | Row 1: name `TextBox` (110×26) + `Save` button (48×26, `AccentBlue`). Row 2: full-row `Load Config ▾` button opening a 150px-wide Overlay-parented flyout | Doesn't register a `Flag` — action panel, not a value-holding widget. Each flyout row is `<config name>` + a small `×` delete glyph (`AccentRed`); clicking the name loads that config (`Library:LoadConfig`) and closes the flyout, clicking `×` deletes it (`Library:DeleteConfig`) and refreshes the flyout in place, staying open. `config.Callback` fires as `(action, name, ok, err)` for `"save"`/`"load"` so a host script can toast the result. Unlike Dropdown/MultiDropdown/ColorPicker, its option rows are rebuilt fresh on every open (`rebuildRows()`) rather than fixed at widget-creation time, since the saved-config set can change between opens |
...
## 14. Floating Menu Toggle Button + Menu Visibility
